...
const int print_script_noexpand =
    0x2;  //!< Printer flag instructing the printer to suppress indenting of
          //!< XML. See print() function.
...
inline OutIt print_data_node(OutIt out,
  ...
  if ((flags & print_script_noexpand) > 0 && node->parent() &&
      internal::compare(node->parent()->name(), node->parent()->name_size(),
                        "script", 6, false)) {
  ...
  if (node->value_size() == 0 && !node->first_node()) {
    ...
    *out = Ch('>'), ++out;
    ...
    if (!child) {
      ...
      if ((flags & print_script_noexpand) > 0 && node &&
          internal::compare(node->name(), node->name_size(), "script", 6,
                            false)) {
        out = copy_chars(child->value(), child->value() + child->value_size(),
                         out);
      } else {
        out = copy_and_expand_chars(
            child->value(), child->value() + child->value_size(), Ch(0), out);
